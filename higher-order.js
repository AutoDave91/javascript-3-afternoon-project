...
//Code Here
let postTaxPrices = prices.map(function(element, i, arr){
  return element*1.07;
});
...
//Code Here
let totalPopulation = populations.reduce(function(total, num, i, arr){
  return total += num;
})
...
let myStrongest = monstersInYourPocket.filter(function(element, i, arr){
  let {CP} = challenge;
  return monstersInYourPocket.map(function(){
    if(challenge > 200){
      return;
    } 
  })
